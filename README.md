...
The final verification result is generated by CBLDiff after executing the same 70 test cases against both the original COBOL implementation and the modernized Java implementation.

- [View final verification result](data/verification_result.json)
- [View execution summary](data/execution_summary.json)
- [View divergence report](data/divergence_report.json)
...
```mermaid
flowchart LR
    A["Legacy COBOL"] --> B["Rule Miner"]
    B --> C["15 Business Rules"]
    C --> D["Test Synthesizer"]
    D --> E["70 Deterministic Tests"]

    E --> F["COBOL Execution"]
    E --> G["Java Execution"]

    F --> H["Field-by-Field Comparison"]
    G --> H

    H --> I["Parity Analysis"]
    I --> J["Critical-Rule Gate"]
    J --> K{"Verified?"}
